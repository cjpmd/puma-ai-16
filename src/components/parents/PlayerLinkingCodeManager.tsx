...
export const PlayerLinkingCodeManager = ({ playerId, playerName }: PlayerLinkingCodeManagerProps) => {
  ...
  const handleGenerateCode = async () => {
    if (!playerId) {
      toast({
        title: "Error",
        description: "Player ID is required to generate a linking code.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      try {
        const { data: functionData, error: functionError } = await supabase
          .rpc('generate_player_linking_code', { player_id: playerId });
        
        if (!functionError && functionData) {
          // Use the code generated by the database function
          setLinkingCode(functionData);
          toast({
            title: "Code Generated",
            description: "New linking code has been generated using database function.",
          });
          setIsGenerating(false);
          return;
        }
      } catch (fnError) {
        console.log("Database function not available, falling back to client-side generation:", fnError);
      }
