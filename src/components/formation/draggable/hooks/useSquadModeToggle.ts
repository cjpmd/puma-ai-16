...
export const useSquadModeToggle = ({
  ...
  useEffect(() => {
    console.log(`formation/useSquadModeToggle - Squad mode: ${squadMode}, can exit: ${canExitSquadMode}, players: ${squadPlayers.length}`);
  }, [squadMode, canExitSquadMode, squadPlayers.length]);
  
  const toggleSquadMode = () => {
    if (!squadMode || canExitSquadMode) {
      console.log(`formation/useSquadModeToggle - Toggling squad mode from ${squadMode} to ${!squadMode}`);
      setSquadMode(!squadMode);
    } else {
      console.log(`formation/useSquadModeToggle - Cannot exit squad mode, need at least ${minRequiredPlayers} player(s)`);
    }
  };
