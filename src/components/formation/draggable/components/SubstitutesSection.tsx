    ...
    <div className="mt-6">
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold">Substitutes ({substitutions.length}/{maxSubstitutes})</h3>
          {selectedPlayerId && substitutions.length < maxSubstitutes && (
            <button 
              onClick={handleAddSubstituteClick}
        ...
        <div className="substitutes-area min-h-[100px]">
          {substitutions.length === 0 ? (
            <div 
              className="h-20 border-2 border-dashed border-gray-300 rounded-md flex items-center justify-center text-gray-500 text-sm"
            >
              {selectedPlayerId 
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
              ...
              {substitutions.map(({ slotId, player, position }) => (
                ...
                >
                  <div 
                    className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-white mb-1 cursor-move"
                    draggable
                    onDragStart={(e) => {
                  ...
                  {/* Render custom or default substitution indicator */}
                  {renderSubstitutionIndicator ? (
