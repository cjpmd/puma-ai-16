...
export const FixtureCard = ({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: existingScores
      } = await supabase.from('fixture_team_scores').select('*').eq('fixture_id', fixture.id);
      if (!existingScores || existingScores.length === 0) {
        ...
        const {
          error: insertError
        } = await supabase.from('fixture_team_scores').insert(defaultScores);
        if (insertError) {
          console.error('Error creating default scores:', insertError);
        }
      }
      const [timesResponse, scoresResponse] = await Promise.all([supabase.from('fixture_team_times').select('*').eq('fixture_id', fixture.id).order('team_number'), supabase.from('fixture_team_scores').select('*').eq('fixture_id', fixture.id).order('team_number')]);
  ...
  return <>
      <Card className="hover:bg-accent/50 transition-colors">
        <FixtureCardHeader fixture={fixture} isCalendarOpen={isCalendarOpen} setIsCalendarOpen={setIsCalendarOpen} onEdit={onEdit} onDelete={onDelete} onTeamSelection={() => setIsTeamSelectionOpen(true)} onDateChange={onDateChange} />
        <CardContent onClick={() => onEdit(fixture)} className="cursor-pointer">
          ...
          {!isLoading && teamData && <TeamScores scores={teamData.scores} times={teamData.times} outcome={fixture.outcome} fixture={{
      ...
      <TeamSelectionDialog isOpen={isTeamSelectionOpen} onOpenChange={setIsTeamSelectionOpen} fixture={fixture} />
    </>;
};